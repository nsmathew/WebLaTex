...
\newline\newline
The tokenizer is required for the input to be prepared appropriately for use by BERT and its variants. It divides the input text into tokens based on a predefined set of rules. These tokens are subsequently transformed into numerical representations and tensors, along with any extra inputs needed by the model to complete the process. Taking a single tweet as an example the behaviour of the tokenizer is described below. 
