...
Next, error analysis is performed on the misclassified tweets. Table \ref{tab: error_tweets} shows sample tweets wrongly classified by the 3 models. Looking at tweets classified incorrectly as non-complaints for BERTweet and DistilBERT some of the possible explanations are provided. The model's inability to detect inherent sarcasm with limited context used in the language (no. 1), very limited text (no. 2), combining a lower form of complaining and asking a question (no. 5) and use of rhetorical questions (no. 6). BERT Tiny seems to struggle with tweets even though they have several indicators of a complaint (no. 9 and 10). For the tweets incorrectly classified as complaints, there are instances of tweets that have words generally associated with complaining such as 'waiting' and 'issue' (no. 3 and 4), 
