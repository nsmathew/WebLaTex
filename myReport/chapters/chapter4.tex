...
Finally, looking at some of the key statistics of the tweets dataset in Table \ref{tab: tweets_statistics}, we find complaint tweets to have a higher average tweet length both in terms of characters and tokens at 96.7 and 19.7 respectively. The random tweets and replies have a token count that is lower by 30\% while the rest of the tweets that are not complaints have 14.6 average tokens or 25\% lower than complaint tweets. This could be due to individuals using various additional linguistics expressions to articulate their frustration or disappointment or to convey a Face Threatening Act against the entity to whom the complaint is directed. 
