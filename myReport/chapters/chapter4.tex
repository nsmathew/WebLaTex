...
The best-performing model was found to be BERTweet with an F1 score of 0.908 and accuracy of 0.934 as shown in Table \ref{tab: model_mean_metrics}. BERTweet being pre-trained on a corpus of 850M tweets seems to give it a significant advantage in capturing the nuances of social media posts including informal language, typographic errors, use of slang and expressive lengthening and more specific characteristics of tweets such as the use of shorter messages, abbreviations and hashtags. RoBERTa was the next best performing model with an F1 of 0.879 and accuracy of 0.914. It was followed by BERT base and DistilBERT with F1 scores of 0.865 and 0.863 respectively. With these models having a standard deviation of \pm 0.02 to 0.03 the 
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|X|X|X|X|X|X|}
        ...
        \hline
    \end{tabularx}
    \caption{Mean performance metrics for all models after nested cross-validation for finetuning and testing. The highest scores are in bold. \(\uparrow\) is the best performing and \(\downarrow\) is the worst performing model. Numbers in () are the results from \cite{jin_complaint_2020}.}
