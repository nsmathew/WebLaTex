...
The performance of BERT and its variants on the classification task will be explored as part of the experiments. BERT is based on the transformers network architecture \cite{vaswaniAttentionAllYou2023a}. Using transformers for the text classification has a number of advantages over previous dominant methods such as Recurrent Neural Networks (RNN) Long Short Term Memory (LSTM). BERT \cite{devlinBERTPretrainingDeep2018} further improved on \cite{vaswaniAttentionAllYou2023a} by introducing
