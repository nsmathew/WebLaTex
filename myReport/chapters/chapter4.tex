...
Looking at the models that are considered lightweight based on their architecture and number of parameters, DistilBERT performs the best, followed by MobileBERT and AlBERT with F1 scores of 0.863 (sd: $\pm$0.02), 0.841 (sd: $\pm$0.02) and 0.827 (sd: $\pm$0.05) respectively. BERT Tiny, the smallest model used in the experiments shows a significant gap in performance with an F1 score of only 0.662 (sd: $\pm$0.04) or lower by 27.6\%. There appears to be a significant performance penalty in this case from the reduced model size although the model is expected to perform better in the context of a knowledge distillation teacher \cite{turcWellReadStudentsLearn2019} which has not been tested here. In Figure \ref{fig: model_size_vs_perf} the relative difference in performance of the models against BERTweet and their corresponding model sizes are shown. ALBERT and MobileBERT while having significantly lower model sizes by 90\% and  have a 
