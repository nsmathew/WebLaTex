...
The best score achieved by \cite{jin_complaint_2020} using BERT Base was 0.882 when using all domains for finetuning and the 'Other' domain for testing. While BERTweet performs best for the same combination the performance is significantly higher at 0.951. This is likely due to the same advantages as described earlier for BERTweet Base being pre-trained on Twitter data. The predictive performance when finetuning with domains with very low tweet volumes is also higher for BERTweet. 'Cars' 
