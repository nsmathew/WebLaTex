...
The test losses are for the 6 runs given Table \ref{tab: test_loss} while the graphs for the performance scores from inference are illustrated in Figure \ref{fig: deep_dive_results}. As expected, the test loss is lower for most the of 6 runs for BERTweet Base compared to the other two models. We find DistilBERT Base having losses that are closer to BERT Base in 2 runs but otherwise are generally higher. There is a considerable difference for BERT Tiny which reflects in the predictive performance scores discussed earlier. Referring to Figure \ref{fig: deep_dive_results}, BERT Tiny exhibits a pattern that differs from the other 2 models. All the performance metrics follow similar trends as the testing progresses. Based on the confuson matrix for each run, it is found that for BERT Tiny true values increase and decrease togther while The confusion matrix for each run is detailed in sec: conf_matrix_test_runs 
...
\begin{table}
    ...
    \end{tblr}
    \caption{Sample tweets which have been misclassified by the 3 selected models. Tweets in the \colorbox{Mercury}{lighter shade of grey} are misclassified as complaints while rest are misclassified as not complaints.}
