...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{On the left is a heatmap of the log of the ratio of the train to test data for every combination executed during the experiments. The best-performing combination using AUC scores is highlighted with '$\bullet$'. Denoted next to the y-axis labels is the number of rows used for the train dataset for that domain. On the right is a plot of the no. of rows for each domain against the performance when that domain is used for finetuning.}
    \label{fig: cross_domain_pair}
\end{figure}

Figure \ref{fig: cross_domain_train_domain} shows how the number of instances or tweets used for the finetuning process affects the mean performance from the testing carried out against all other domains. At a macro level, it appears, that the more data that is available for finetuning the better the overall performance. However, certain domains such as other, apparel and software show a deviation from this trend. There possibly could be inherent characteristics of the tweets from these domains which could be affecting their performance. 

While the convention is to use significantly higher data for training compared to testing, the cross-domain experiments involve datasets skewed in favour of testing data volume. To analyse this, Figure \ref{fig: cross_domain_heat} shows a heatmap of the ratio of train to test data for every combination of domains used in this experiment set. The white ’•’ indicates the best-performing combinations in terms of AUC scores. We find 