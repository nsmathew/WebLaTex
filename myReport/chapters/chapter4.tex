...
Analysis of the hashtags in the complaints reveals the use of hashtags primarily as an expression of frustration as shown in Figure \ref{fig: top_hashtags}. Hashtags like \texttt{\#nothappy}, \texttt{\#fail}, \texttt{\#disappointed} are examples. Aside from such tags, there are a number of tags that are brand-specific or product specific which are not shown here. As per Twitter\footnote{\url{https://help.twitter.com/en/using-twitter/how-to-use-hashtags#}}, users utilize the symbol "\#" (hashtag) preceding a keyword or phrase significant to the context in their tweet to classify those tweets, facilitating their visibility in Twitter searches. Clicking or tapping on a hashtagged term within any message reveals additional Tweets containing the same hashtag. Hashtags can be inserted at any point within a Tweet. Frequently, words marked with hashtags that attain significant popularity transform into trending topics. However, purely in terms of volumes, tweets with hashtags in the dataset are quite limited, especially for the complaints. There are only 149 complaint tweets out of a total of 459 tweets that include hashtags. This is about 12\% of all complaint tweets. Excluding the random tweets and replies, the tweets that are not complaints and have hashtags are only 67. Overall average hashtags for tweets with at least one hashtag is 1.56. Based on this, while the availability of has
