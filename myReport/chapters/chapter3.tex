...
\begin{figure}
    \centering
    \captionsetup{font=small}
    \includegraphics[width=\textwidth]{figures/token_hist.png} % Replace with your image file name
    \caption{The distribution of the no. of tokens for the full dataset of 3,449 tweets before tokenization.}
    \label{fig: token_hist}
\end{figure}
\section{Data tokenisation}
For tokenization, the tokenizer provided by the \textit{transformers} library is used. The distribution of the number of tokens in the tweets from the pre-processed data from \cite{jinModelingSeverityComplaints2021} before applying the model-specific tokenization is shown in Figure \ref{fig: token_hist}. We have over 95\% of the tweets having 29 tokens or less. The tokenization process provided by the \textit{transformers} library using \textit{AutoTokenizer} allows for the input to be prepared appropriately for use by BERT and its variants.
\begin{figure}
    \centering
    \captionsetup{font=small}
    \includegraphics[width=\textwidth]{figures/token_pp_hist.png} % Replace with your image file name
    \caption{The token count distribution for the full dataset post tokenization.}
...
\begin{figure}[htbp]
    ...
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{token_pp_hist.png}
