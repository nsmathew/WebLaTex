...
\begin{table}
    ...
    \begin{tblr}{
      width = \linewidth,
      colspec = {Q[92]Q[152]Q[158]Q[108]Q[152]Q[158]Q[108]},
      cells = {c},
      row{even} = {Mercury},
      row{1} = {Nobel},
      row{2} = {Nobel},
      cell{1}{1} = {r=2}{},
      cell{1}{2} = {c=3}{0.418\linewidth},
      cell{1}{5} = {c=3}{0.418\linewidth},
      vlines,
      hline{1,3-9} = {-}{},
      hline{2} = {2-7}{},
    }
    \textbf{Test Run } & \textbf{Test Loss }    &                          &                    & \textbf{MCC }          &                          &                    \\
                       & \textbf{BERTweet Base} & \textbf{DistilBERT Base} & \textbf{BERT Tiny} & \textbf{BERTweet Base} & \textbf{DistilBERT Base} & \textbf{BERT Tiny} \\
    1                  & 0.21700                & 0.22481                  & 0.4918             & 0.852                  & 0.840                    & 0.530              \\
    2                  & 0.20056                & 0.24783                  & 0.50118            & 0.852                  & 0.796                    & 0.455              \\
    3                  & 0.23015                & 0.2619                   & 0.48807            & 0.846                  & 0.744                    & 0.453              \\
    4                  & 0.23572                & 0.25394                  & 0.48178            & 0.860                  & 0.812                    & 0.537              \\
    5                  & 0.25255                & 0.30100                  & 0.53076            & 0.830                  & 0.767                    & 0.425              \\
    6                  & 0.17506                & 0.26712                  & 0.48810            & 0.902                  & 0.789                    & 0.551              
    \end{tblr}
    \caption{Test loss and Matthews Correlation Coefficient (MCC) from the inference phase for the 6 outer loop iterations for the 3 selected models.}
    \label{tab: test_loss}
\end{table}
...
The test losses for the 6 runs are given in Table \ref{tab: test_loss} while the graphs for the performance scores from inference are illustrated in Figure \ref{fig: deep_dive_results}. As expected, the test loss is lower for most the of 6 runs for BERTweet Base compared to the other two models. We find DistilBERT Base having losses that are closer to BERT Base in 2 runs but otherwise are generally higher. There is a considerable difference for BERT Tiny which reflects in the predictive performance scores discussed earlier. Table \ref{tab: test_loss} also include the MCC scores. MCC scores range from [-1, +1] with -1 being perfect misclassification while +1 indicates perfect classification and depend on the model making correct predictions on both positive and negative classes \cite{chiccoAdvantagesMatthewsCorrelation2020}. The MCC score for BERTweet across the 6 test runs and mean of 0.857 (sd: $\pm$0.02) gives a more conservative picture when compared to the F1 score discussed earlier for the model. DistilBERT has \\
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Volume of datasets used for the experiments.}
        \label{fig: cross_domain_heat}
    \end{subfigure}
    \caption{On the left is a plot of the no. of tweets for each domain against the average performance when that domain is used for finetuning. On the right is a heatmap of the number of tweets used for every combination tested during the experiments. The best-performing combination using AUC scores is highlighted with '$\bullet$'.}
...
To analyse this further, the performance of the combinations of domains concerning the overall data volume is considered. Figure \ref{fig: cross_domain_heat} shows a heatmap of the number of tweets used for all the experiments with the white ’•’ indicating the best-performing combinations in terms of AUC scores. We find the best performance is not always gained from datasets with the highest volumes. There are possibly other factors influencing the performance include the nature of the tweets and how informative they are, the ratio of training data vs testing data volumes among other.