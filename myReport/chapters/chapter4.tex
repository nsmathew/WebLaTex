...
The best performance is where we use all the data except the 'Other' domain tweets for finetuning and the 'Other' domain for testing. It has a relatively high discriminative ability represented by an AUC score of 0.908. This combination also has an F1 score 0.951, indicating a good balance between its precision and recall as well. The use of diverse domains for finetuning likely has a positive impact on how the models learn for the downstream task. In general, the results for the experiments where all data except one domain is used for finetuning are consistently higher consistently when compared to using just a single domain for finetuning. For single domain finetuning cases, for some of the domains we have relatively close performance to the all domain finetuning even though the volume of training data is significnatly lower. Domains such as 'Apparel', 'Services', 'Transport' and 'Electronics' have AUC and F1 scores of over 0.8 despite the volume of data used for finetuning being limited to a range of 247 - 335 tweets. Single-domain finetuning instances with extremely low volumes have resulted in an AOC of 0.5 or close to 0.5 which means the metric thinks the model's prediction ability is as good as random chance. These include the domains of 'Cars', 'Food & Beverage' and 'Others' with only 93, 127 and 128 tweets available for finetuning respectively.
