...
Table \ref{tab: model_mean_metrics} presents the prediction metrics sourced from \cite{jin_complaint_2020}, enclosed within '[ ]', which serves as the established baseline performance for this task. Despite the variation in nested cross-validation configuration, as outlined in the preceding chapter, a level of preliminary comparison becomes feasible. In terms of F1 scores, RoBERTa shows marginal improvement, while BERT base and ALBERT display no noticeable improvement. Nonetheless, when considering the task against the baseline, BERTweet emerges as the optimal predictor, achieving an impressive F1 score of 0.908.
...
The Figure \ref{fig: model_size_vs_perf} displays the relative variance in model performance when compared to BERTweet, alongside the corresponding sizes of the models. Notably, ALBERT and MobileBERT exhibit relatively lower performance discrepancy of only 8.9\% and 7.4\%, despite having considerably reduced model sizes by 90\% and 77\% respectively. Similarly, DistilBERT demonstrates a performance deficit of merely 4.9\%, akin to that of BERT base, while having a 44\% reduction in model size. This correlation suggests that the knowledge distillation compression technique applied during the pretraining phase could be playing a pivotal role in enhancing DistilBERT's predictive capabilities \cite{sanhDistilBERTDistilledVersion2020}.
